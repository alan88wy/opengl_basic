...
void Mesh::CreateMesh(GLfloat* vertices, unsigned int* indices, unsigned int noOfVertices, unsigned int noOfIndices) 
{
    ...
    //                            const void* pointer);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, 0); // define an array of generic vertex attribute data
    glEnableVertexAttribArray(0); // void glEnableVertexAttribArray(GLuint index); Enable or disable generic vertex attribute array
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, (void*)(sizeof(vertices[0]) * 3)); // define an array of generic vertex attribute data
    glEnableVertexAttribArray(1); // void glEnableVertexAttribArray(GLuint index); Enable or disable generic vertex attribute array
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(vertices[0]) * 8, (void*)(sizeof(vertices[0]) * 5)); // define an array of generic vertex attribute data
